...
def getOwnTrainedModelVAE(architecture, ckpt, batch_size = 8):

    ckpt = Constants.MAIN_PATH + "/" + ckpt

    if (ckpt == None):
        logging.error("Model Not Found: " + ckpt)
        sys.exit(1)

    logging.info("Loading " + architecture + "in checkpoint: " + ckpt)
    return TrainedModel(
        configs.CONFIG_MAP[architecture],
        batch_size=batch_size,
